Vagrant.configure(2) do |config|
  ...
  # synced folder
  config.vm.synced_folder '.', '/uottajudge', type: 'rsync',
  ...
  config.vm.provision 'shell', inline: <<-EOS
    ...
    echo "host  uottajudge vagrant  ::1/128      trust" >> "$pg_auth_file"
    echo "host  uottajudge vagrant  127.0.0.1/32 trust" >> "$pg_auth_file"
