...
from uottajudge.models import *
from uottajudge.constants import *
import uottajudge.settings as settings
import uottajudge.utils as utils
import uottajudge.stats as stats
from flask import (
    redirect,
    render_template,
    request,
...
def admin():
    ...
    return render_template(
        ...
        items=items,
        votes=len(decisions),
        annotatorsLen=len(annotators),
        countsLen=len(item_counts),
        setting_closed=setting_closed,
        top3=[obj.name for obj in sorted(items, key=lambda x: x.mu, reverse=True)[:3]]
    )
